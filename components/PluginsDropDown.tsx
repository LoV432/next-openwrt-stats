			...
			<DropdownMenu>
				<DropdownMenuTrigger asChild>
					<Button variant="outline" className="gap-1">
						<PlugIcon />
						<span className="hidden md:block">Plug-ins</span>
					...
					{wireguardInterfaces?.length && wireguardInterfaces.length > 0 && (
						<>
							<DropdownMenuSub>
								...
								<DropdownMenuPortal>
									<DropdownMenuSubContent>
										{wireguardInterfaces?.map((device, index) => (
											<Fragment key={device.interface}>
												<DropdownMenuItem
													key={device.interface}
													onClick={() => {
														setSelectedWireguardInterface(device.interface);
														wireguardDialogState.setIsOpen(true);
													}}
												>
													{device.interface}
												</DropdownMenuItem>
												{index !== wireguardInterfaces.length - 1 && (
													<DropdownMenuSeparator />
												)}
											</Fragment>
										))}
